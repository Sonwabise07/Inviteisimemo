...
def create():
    ...
    f     = request.form
    token = uuid.uuid4().hex[:12]
    is_draft = f.get('save_mode') == 'draft' or f.get('save_mode_btn') == 'draft'

    # Draft saves don't require a title — give a friendly placeholder so the
    # event is findable in My Events.
    event = _build_event_from_form(f, request.files, token)
    if is_draft and not (event.event_title or '').strip():
        event.event_title = 'Untitled invite'
    event.user_id = current_user.id
    ...
    # Drafts are never auto-published; only an active subscriber's full submit is
    event.is_live = (not is_draft) and _has_active_sub(current_user)
    db.session.commit()

    if is_draft:
        _audit('event_draft_save', f'token={token}')
        flash('Draft saved ✓ — pick up any time from My Events.', 'success')
        return redirect(url_for('edit_event', token=token))
...
def edit_event(token):
    ...
    if request.form.get('save_mode') == 'draft' or request.form.get('save_mode_btn') == 'draft':
        _audit('event_draft_save', f'token={token}')
...
def duplicate_event(token):
    ...
    new_event = Event(
        ...
        accent_colour_2=src.accent_colour_2,
        design=src.design,
        font=src.font,
        pattern=src.pattern,
        pattern_opacity=src.pattern_opacity,
        decorations=src.decorations,
        effect=src.effect,
        photo_filter=src.photo_filter,
        ...
        spotify_url=src.spotify_url,
        whatsapp_group=src.whatsapp_group,
        hashtag=src.hashtag,
        greeting_lang=src.greeting_lang,
...
def _finalise_payment(user, kind: str, token: str, pf_payment_id: str = '',
    ...
    the sandbox success shortcut. Idempotent-ish: safe to call once per ITN."""
    db.session.add(Payment(
        user_id=user.id, pf_payment_id=pf_payment_id, pf_sub_token=pf_sub_token,
        amount=float(amount) if amount else 0, status='complete',
        kind='single_invite' if kind == 'single' else 'subscription',
        event_id=None,
    ))
    if kind == 'single':
        ev = Event.query.filter_by(token=token, user_id=user.id,
...
def subscribe_checkout():
    ...
    # Dev escape — skip real PayFast while testing
    if app.config.get('PAYFAST_SANDBOX'):
        return render_template('dev_payment.html', kind='sub',
                               amount=price, return_token=return_token,
                               item='Monthly Subscription (unlimited invites)')
...
def single_invite_checkout(token):
    ...
    # Dev escape — skip real PayFast while testing
    if app.config.get('PAYFAST_SANDBOX'):
        return render_template('dev_payment.html', kind='single',
                               amount=price, return_token=token,
                               item='Unlock: ' + event.event_title[:50])
...
@app.route('/dev/pay/finalize', methods=['POST'])
@login_required
def dev_pay_finalize():
    """Sandbox-only: the 'Simulate payment' button on the dev escape page.
    Mimics a successful PayFast payment so the whole flow can be tested
    without real money. Disabled entirely when PAYFAST_SANDBOX is off."""
...
def payment_success():
    ...
    # Finalisation happens in the PayFast ITN (production) or the dev escape
    # (sandbox) — NOT here. This page only reports/awaits status.
    event = None
    if token:
...
def _build_csv_response(event) -> Response:
    output = io.StringIO()
...
def export_csv(token):
    ...
    if not _can_manage(event):
        abort(403)
    return _build_csv_response(event)
